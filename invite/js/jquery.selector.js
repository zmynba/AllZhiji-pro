...
(function($) {
	...
	$('body').append('<style>' +
		...
		'.sel-box .btn1{overflow:hidden;width:44px;height:44px;float:left;margin:22px 20px;}' +
		'.sel-box .btn1 img{float:left;width:100%;height:100%;}' +
		'.sel-box .ok{float:right;}' +
		'.sel-box .name{color:white;text-align:center;line-height:44px;font-size:30px;padding:22px 0;}' +
		'.sel-con{background:white;}' +
		// '.sel-con .border{height:68px;border:solid 1px #EBEBEB;border-width:1px 0;position:fixed;bottom:142px;left:0;right:0;pointer-events:none;}' +
		'.sel-con .border{height:68px;background: rgba(0,0,0,.1);border-width:1px 0;position:fixed;bottom:146px;left:0;right:0;pointer-events:none;}' +
		...
		'.sel-con .scroll{-webkit-overflow-scrolling:touch;height:360px;overflow:auto;box-sizing:border-box;padding:144px 0;width:200%;padding-right:100%;}' +
		'.sel-con .ele{font-size:30px;color:#b2b2b2;height:72px;line-height:72px;}' +
	...
	$.scrEvent = function(params) {
		...
		$('.sel-box .btn').css('background', (params.defBg || '#fda626'));
	...
	$.scrEvent2 = function(params) {
		...
		$(evEle).attr('readonly', 'readonly');
		eleName != '' ? eleName = '<div class="cell" style="font-size:28px;color:#b2b2b2;">' + eleName + '</div>' : eleName = '';
		eleName2 != '' ? eleName2 = '<div class="cell" style="font-size:28px;color:#b2b2b2;">' + eleName2 + '</div>' : eleName2 = '';
		...
		$(evEle).on(type, function() {
			...
			$('.sel-con .scroll').scroll(function() {
				var that = $(this);
				// 数值显示
				var scTop = $(this)[0].scrollTop + 36;
				var scNum = Math.floor(scTop / 72);
	...
	$.dateSelector = function(params) {
		...
		$(evEle).on(type, function() {
			...
			$('.sel-con').find('.elem').eq(0).find('.ele').each(function() {
				if($(this).text() == year) {
					$(this).parents('.scroll')[0].scrollTop = $(this).index() * 72;
				}
			});
			$('.sel-con').find('.elem').eq(1).find('.ele').each(function() {
				if($(this).text() == month) {
			...
			// 进行传值
			$('.sel-box .ok').click(function() {
				$(evEle).attr('data-sel01', scText);	//年
				$(evEle).attr('data-sel02', scText2);	//月
				$(evEle).attr('data-sel03', scText3);	//日
				$(evEle).attr('data-sel04', scText4);	//小时
				$(evEle).attr('data-sel05', scText5);	//分钟
				//单个传递
